...
## 📂 Package Management
- **APT (Advanced Package Tool)** – front-end for `dpkg`  
- Common commands:  
...
## 🚀 Debian SSH Setup & Configuration
...
### 4️⃣ Install Essential Packages
```bash
apt install bash* vim net-tools -y
```

### 5️⃣ Install SSH Packages
```bash
apt install openssh-server openssh-client ssh
```

### 6️⃣ Edit SSH Configuration
```bash    
vim /etc/ssh/sshd_config
```
...
## 🌐 Set `Static IP` Configuration on Debain
...
```
---
# 🐚 Zsh Installation & Configuration on Debian

## 1️⃣ Check Current Shell
```bash
echo $SHELL
```
## 2️⃣ List Available Shells
```bash
cat /etc/shells
```
## 3️⃣ Install Zsh with Plugins
```bash
apt install zsh zsh-autosuggestions zsh-syntax-highlighting 
```
## 4️⃣ Verify Zsh Installation
```bash
zsh --version
```

## 5️⃣ Locate Zsh Binary
```bash
which zsh
```
...
## 7️⃣ Verify User Shell in passwd File
```bash
grep zsh /etc/passwd
```

## 8️⃣ Configure Zsh (edit config file)
```bash
nano ~/.zshrc
```
## Write in zshrc file:
...
```
---
## 👤 Enable Zsh for User `armour`

## Reload Zsh Configuration
```bash
source ~/.zshrc
```
## Switch to User `armour`
```bash
su - armour
```
## Set Zsh as Default Shell for `armour`
```bash
chsh -s $(which zsh)
```
## Edit Zsh Configuration for `armour`
```bash
nano /home/security/.zshrc
```